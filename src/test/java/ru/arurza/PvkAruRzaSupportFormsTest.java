...
public class PvkAruRzaSupportFormsTest {
    ...
    static Stream<Arguments> aruRzaDataProvider() {
        return Stream.of(
                Arguments.of("/support", List.of("Контактное лицо (Ф.И.О) *", "Организация *", "Город", "Эл.почта *", "Телефон *", "В чем состоит вопрос", "Принимаю условия пользовательского соглашения (см. «Поддержка») *")),
                Arguments.of("/support/demoversion", List.of("Название организации *", "Сфера деятельности *", "Город *", "Адрес", "Контактное лицо (Ф.И.О) *", "Должность", "Эл.почта *", "Телефон *", "Дополнительная информация", "Принимаю условия пользовательского соглашения (см. «Поддержка») *"))
        );
    }
